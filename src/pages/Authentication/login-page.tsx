...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (userData) {
      if (userData.roles.includes("Supplier")) {
        navigate(from || redirectPath || previousPath || "/supplier-dashboard");
      } else {
        navigate(from || redirectPath || previousPath || "/buyer-dashboard");
      }
    }

    return () => clearTimeout(timeoutKey);
  }, [errorMessage, userData, navigate, redirectPath, previousPath, from]);
  ...
  const onSubmit = async (values: z.infer<typeof loginSchema>) => {
    ...
    try {
      ...
      if (response.status === HttpStatusCode.Ok) {
        ...
        if (userData.roles.includes("Supplier")) {
          navigate(redirectPath || previousPath || "/supplier-dashboard");
        } else {
          navigate(redirectPath || previousPath || "/buyer-dashboard");
        }
